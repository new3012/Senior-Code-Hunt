...
export default function AdminApp(){
 ...
 return <main className="admin-shell">
  ...
  <section><h2>แผนภารกิจและคำใบ้</h2><div className="mission-editor">{data.missions.map(m=><details key={String(m.id)}><summary>DAY {String(m.day)} · {String(m.title)} <b>{String(m.kind)}</b></summary>{[["title","ชื่อภารกิจ"],["task","โจทย์"],["snippet","โค้ด"],["answer","คำตอบ"],["help_text","คำใบ้ช่วย"],["clue","คำใบ้ที่ได้"]].map(([field,label])=><label key={field}>{label}<textarea defaultValue={String(m[field]??m[field==="help_text"?"help":""]??"")} onBlur={e=>patch({id:m.id,field,value:e.target.value})}/></label>)}</details>)}</div></section>
