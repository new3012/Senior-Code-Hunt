...
export default function AdminApp(){
 ...
 if(!data)return <main className="admin-login"><div><p className="eyebrow">RESTRICTED AREA</p><h1>ADMIN<br/><em>CONTROL ROOM</em></h1><input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&login()} placeholder="รหัสผ่านผู้ดูแล"/><button onClick={login}>เข้าสู่ระบบ</button>{error&&<p className="error">{error}</p>}</div></main>;
 return <main className="admin-shell">
  ...
  <section><h2>ผู้เล่น ({data.players.length}/7)</h2><div className="admin-player-scroll"><div className="admin-grid">{data.players.map(p=><article key={p.id}><strong>{p.nickname}</strong><span>{p.completed}/9 ภารกิจ</span><small>ล่าสุด {new Date(p.lastSeenAt).toLocaleString("th-TH")}</small><button onClick={()=>reset(p.id)}>ล้างความคืบหน้า</button></article>)}</div></div></section>
