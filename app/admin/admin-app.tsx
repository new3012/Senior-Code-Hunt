...
export default function AdminApp(){
 ...
 return <main className="admin-shell">
  ...
  <section><h2>หลักฐานที่ผ่านแล้ว</h2><div className="evidence-grid">{data.completions.map(c=><article key={String(c.id)}>{Boolean(c.evidencePath)&&<img src={`/api/admin/evidence/${String(c.id)}`} alt="หลักฐานภารกิจ"/>}<b>{String(c.nickname)} · ภารกิจที่ {String(c.day)}</b><span>{String(c.method)}</span><time>{new Date(String(c.passedAt)).toLocaleString("th-TH")}</time></article>)}</div></section>
 </main>;
}
