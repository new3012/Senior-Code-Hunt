...
import { elapsedBangkokDays } from "@/lib/time";
...
export async function POST(request:Request){
  ...
  const elapsed=elapsedBangkokDays((await settings()).startDate);
  if(!mission||mission.kind==="photo"||next?.id!==mission.id||Number(mission.unlock_offset)>elapsed)return NextResponse.json({error:"ยังไม่ถึงภารกิจนี้"},{status:409});
