...
export async function POST(request:Request){
  ...
  if(existing[0]?.status==="pending")return NextResponse.json({error:"รูปนี้กำลังรอพี่รหัสตรวจอยู่"},{status:409});
  ...
  return NextResponse.json({ok:true,pending:true});
    ...
    return NextResponse.json({error:"ระบบอัปโหลดรูปมีปัญหา กรุณาลองใหม่อีกครั้ง"},{status:500});
  }
}
