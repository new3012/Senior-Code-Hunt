...
export default function HuntApp(){
 ...
 <section className={`bonus-wrap special-mission-wrap ${state.bonus?.unlocked?"is-unlocked":"is-locked"}`}>
  <div className="special-mission-heading"><div><p className="eyebrow">{state.bonus?.unlocked?"UNLOCKED · SPECIAL MISSION":"SPECIAL MISSION · SUNDAY 18:00"}</p><h2>ภารกิจพิเศษ</h2></div>{state.bonus?.codeDone&&<span className="special-complete-badge">ผ่านแล้ว</span>}</div>
  {!state.bonus?.unlocked?<div className="special-mission-locked"><span className="special-lock">⌁</span><div><b>เปิดวันอาทิตย์ เวลา 18:00 น.</b><p>ภารกิจพิเศษมี 2 เกม เมื่อผ่านครบจะได้รับคำใบ้พิเศษเพิ่มอีก 1 คำใบ้</p><strong>{String(bonusHours).padStart(2,"0")}:{String(bonusMins).padStart(2,"0")}:{String(bonusSecs).padStart(2,"0")}</strong></div></div>:<MiniGames initialSequenceDone={state.bonus.sequenceDone} initialCodeDone={state.bonus.codeDone} specialClue={state.bonus.specialClue} onSaved={load}/>}
