...
export default function HuntApp(){
 ...
 const load=useCallback(async()=>{const r=await fetch("/api/state",{cache:"no-store"});const data:State=await r.json();setState(data);setCountdown(data.nextMidnightMs??0);setBonusCountdown(data.bonus?.remainingMs??0);if(data.latestUnlock&&localStorage.getItem("hunt_seen_clue")!==data.latestUnlock.passedAt)setRevealed(data.latestUnlock)},[]);
 useEffect(()=>{
  // A pending/completed/different mission must never reuse the previous photo.
  setPhotoFile(null);
 },[state?.current?.id,state?.pendingPhoto]);
 useEffect(()=>{load()},[load]);
 useEffect(()=>{const timer=setInterval(()=>{setCountdown(v=>Math.max(0,v-1000));setBonusCountdown(v=>Math.max(0,v-1000))},1000);return()=>clearInterval(timer)},[]);
 useEffect(()=>{if(state?.bonus&&!state.bonus.unlocked&&state.bonus.remainingMs>0&&bonusCountdown===0)void load()},[bonusCountdown,state?.bonus,load]);
 useEffect(()=>{const refresh=()=>void load();window.addEventListener("focus",refresh);const onVisibility=()=>{if(document.visibilityState==="visible")void load()};document.addEventListener("visibilitychange",onVisibility);const poll=window.setInterval(refresh,20000);return()=>{window.removeEventListener("focus",refresh);document.removeEventListener("visibilitychange",onVisibility);window.clearInterval(poll)}},[load]);
 ...
 const hours=Math.floor(countdown/3600000),mins=Math.floor(countdown%3600000/60000),secs=Math.floor(countdown%60000/1000);
 const bonusHours=Math.floor(bonusCountdown/3600000),bonusMins=Math.floor(bonusCountdown%3600000/60000),bonusSecs=Math.floor(bonusCountdown%60000/1000);
 ...
 <section className="terminal-card"><div className="terminal-head"><span>DAILY_CLUE.EXE</span><div><i/><i/><i/></div></div><div className="terminal-body"><div className="code-line"><span>01</span><code>status: <b>{current?"ENCRYPTED":"SYNCED"}</b></code></div>{notice&&<p className="success">{notice}</p>}{revealed&&<div className="clue-reveal"><p className="mission-badge">ภารกิจที่ {String(revealed.day).padStart(2,"0")} · {revealed.tag}</p><span className="reveal-check">✓</span><h2>{revealed.title}</h2><blockquote>“{revealed.clue}”</blockquote><button onClick={()=>{localStorage.setItem("hunt_seen_clue",revealed.passedAt);setRevealed(null);setNotice("")}}>บันทึกคำใบ้และไปต่อ ↗</button></div>}
 {current?<div className="locked-content"><div className="mission-badge">ภารกิจที่ {String(current.day).padStart(2,"0")} · {current.tag}</div><h2>{current.title}</h2><p>{current.task}</p>{current.snippet&&<pre className="challenge-code">{current.snippet}</pre>}{current.kind==="choice"&&<div className="choice-grid">{current.choices.map(c=><button key={c} className={answer===c?"selected":""} onClick={()=>setAnswer(c)}>{c}</button>)}</div>}{current.kind==="code"&&<input className="answer-input" value={answer} onChange={e=>setAnswer(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitAnswer()} placeholder="พิมพ์คำตอบที่นี่"/>}{current.kind!=="photo"&&<button className="unlock-button" disabled={busy} onClick={submitAnswer}>{busy?"กำลังตรวจ...":"VERIFY & DECRYPT ↗"}</button>}{current.kind==="photo"&&(state.pendingPhoto?<div className="pending-box photo-pending"><span className="pending-icon">⌛</span><div><b>รอพี่รหัสตรวจรูป</b><span>ส่งหลักฐานเรียบร้อยแล้ว เมื่อพี่รหัสอนุมัติ ระบบจะปลดล็อกคำใบ้ให้อัตโนมัติ</span></div><button type="button" onClick={()=>{setPhotoFile(null);load()}}>ตรวจสอบสถานะ</button></div>:<>{state.rejectedPhotoReason&&<div className="photo-rejected"><span>ไม่ผ่าน</span><div><b>เหตุผลจากพี่รหัส</b><p>{state.rejectedPhotoReason}</p><small>แก้ไขตามเหตุผลแล้วถ่ายหรือเลือกรูปใหม่ได้เลย</small></div></div>}<form onSubmit={submitPhoto} className="photo-form"><div className="photo-upload-card"><div className="upload-copy"><b>ส่งหลักฐานภารกิจ</b><span>เลือกได้ว่าจะถ่ายรูปใหม่ หรือเลือกรูปจากเครื่อง</span></div><div className="photo-source-single">
 <label className="photo-source-btn choose-photo-btn"><span>เลือกรูปหรือถ่ายรูป</span><input name="photo" type="file" accept="image/*" onClick={e=>{e.currentTarget.value=""}} onChange={e=>{const f=e.target.files?.[0]??null;if(f)setPhotoFile(f,f.name)}}/></label>
</div>{photoPreview&&<div className="photo-preview-card"><img src={photoPreview} alt="รูปหลักฐานที่เลือก"/><div><b>รูปที่เลือก</b><span>{photoName}</span><button type="button" onClick={()=>setPhotoFile(null)}>เลือกรูปใหม่</button></div></div>}</div><label className="consent"><input type="checkbox" required/><span>ยินยอมให้พี่รหัสดูรูปเพื่อตรวจภารกิจ โดยรูปจะถูกลบภายใน 7 วัน</span></label><button className="photo-submit" disabled={busy}>{busy?"กำลังอัปโหลด...":"ส่งรูปให้พี่รหัสตรวจ ↗"}</button></form></>)}<button className="hint-button" onClick={()=>setHint(!hint)}>{hint?"ซ่อนคำใบ้ช่วย":"ขอคำใบ้ช่วย"}</button>{hint&&<p className="hint">HINT: {current.help}</p>}{error&&<p className="error">{error}</p>}</div>:state.finalAvailable?<div className="locked-content final-mission final-choice-mission"><div className="mission-badge">ด่านสุดท้าย · ปัญหาเชาว์</div><h2>{state.finalMission?.title??"ปัญหาเชาว์"}</h2><p className="final-riddle-text">{state.finalMission?.task}</p><div className="choice-grid final-choice-grid">{(state.finalMission?.choices??[]).map(c=><button key={c} className={finalAnswer===c?"selected":""} onClick={()=>setFinalAnswer(c)}>{c}</button>)}</div><button className="unlock-button" disabled={busy||!finalAnswer} onClick={submitFinal}>{busy?"กำลังตรวจ...":"ตรวจคำตอบ ↗"}</button><button className="hint-button" type="button" onClick={()=>setHint(!hint)}>{hint?"ซ่อนคำใบ้ช่วย":"ขอคำใบ้ช่วย"}</button>{hint&&<p className="hint">HINT: {state.finalMission?.help}</p>}{error&&<p className="error">{error}</p>}</div>:state.finalCompleted?<div className="caught-up final-complete"><span>✓</span><h2>ภารกิจทั้งหมดเสร็จสมบูรณ์</h2><p>ตอนนี้คำใบ้อยู่กับคุณครบแล้ว จงนำคำใบ้ทั้งหมดมารวมกัน และตามหาว่าพี่รหัสของคุณคือใคร</p></div>:<div className="caught-up"><span>✓</span><h2>ทำภารกิจที่เปิดอยู่ครบแล้ว</h2><p>ภารกิจถัดไปเปิดใน</p><strong>{String(hours).padStart(2,"0")}:{String(mins).padStart(2,"0")}:{String(secs).padStart(2,"0")}</strong></div>}</div></section></section>
 <section className={`bonus-wrap special-mission-wrap ${state.bonus?.unlocked?"is-unlocked":"is-locked"}`}>
  <div className="special-mission-heading"><div><p className="eyebrow">{state.bonus?.unlocked?"UNLOCKED · SPECIAL MISSION":"SPECIAL MISSION · SUNDAY 18:00"}</p><h2>{state.bonus?.title??"ภารกิจพิเศษ"}</h2></div>{state.bonus?.codeDone&&<span className="special-complete-badge">ผ่านแล้ว</span>}</div>
  {!state.bonus?.unlocked?<div className="special-mission-locked"><span className="special-lock">⌁</span><div><b>เปิดวันอาทิตย์ เวลา {state.bonus?.unlockTime??"18:00"} น.</b><p>ภารกิจพิเศษมี 2 เกม เมื่อผ่านครบจะได้รับคำใบ้พิเศษเพิ่มอีก 1 คำใบ้</p><strong>{String(bonusHours).padStart(2,"0")}:{String(bonusMins).padStart(2,"0")}:{String(bonusSecs).padStart(2,"0")}</strong></div></div>:<MiniGames initialSequenceDone={state.bonus.sequenceDone} initialCodeDone={state.bonus.codeDone} specialClue={state.bonus.specialClue} sequenceRounds={state.bonus.sequenceRounds} codeAttempts={state.bonus.codeAttempts} onSaved={load}/>}
 </section> <section className="archive-wrap"><button className="archive-toggle" onClick={()=>setArchive(!archive)}><span>ประวัติคำใบ้และแผนภารกิจ</span><b>{archive?"−":"+"}</b></button>{archive&&<div className="archive-grid">{state.missions?.map(m=><article key={m.id} className={m.passedAt?"available":m.released?"waiting":"future"}><span>ภารกิจที่ {String(m.day).padStart(2,"0")}</span><h3>{m.title}</h3><p>{m.passedAt?m.clue:m.released?"เปิดแล้ว — ต้องผ่านภารกิจก่อนหน้าให้ครบ":"ยังไม่ถึงวันปลดล็อก"}</p></article>)}</div>}</section><footer><span>ระบบติดตามพี่รหัส v2.0</span><a href="/admin">ADMIN</a></footer></main>;
}
