...
export async function ensureDb() {
  if (!global.seniorHuntReady) global.seniorHuntReady = (async () => {
    ...
    try {
      ...
      if(Number(versions[0]?.content_version??1)<3){
        await c.query("UPDATE hunt_missions SET unlock_offset=CASE day_number WHEN 1 THEN 0 WHEN 2 THEN 1 WHEN 3 THEN 1 WHEN 4 THEN 2 WHEN 5 THEN 3 WHEN 6 THEN 4 WHEN 7 THEN 5 WHEN 8 THEN 5 WHEN 9 THEN 6 ELSE day_number-1 END");
        ...
        await c.query("UPDATE hunt_settings SET content_version=3 WHERE id=1");
      }
    } finally { c.release(); }
  })();
