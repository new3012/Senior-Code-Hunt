...
export async function ensureDb() {
  if (!global.seniorHuntReady) global.seniorHuntReady = (async () => {
    ...
    try {
      ...
      await c.query(`CREATE TABLE IF NOT EXISTS hunt_completions (id BIGINT AUTO_INCREMENT PRIMARY KEY, player_id CHAR(36) NOT NULL, mission_id INT NOT NULL, method ENUM('answer','ai_photo') NOT NULL, evidence_path VARCHAR(500) NULL, ai_result JSON NULL, passed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY uniq_player_mission(player_id,mission_id), CONSTRAINT fk_completion_player FOREIGN KEY(player_id) REFERENCES hunt_players(id) ON DELETE CASCADE, CONSTRAINT fk_completion_mission FOREIGN KEY(mission_id) REFERENCES hunt_missions(id) ON DELETE CASCADE)`);
