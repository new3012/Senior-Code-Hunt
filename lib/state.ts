...
export async function getPlayerState(id: string) {
  ...
  const { startDate, bonusClue, bonusTitle, bonusUnlockTime, bonusSequenceRounds, bonusCodeAttempts, finalTitle, finalTask, finalHelp, finalChoices } = await settings();
  ...
  const bonusTime=sundayBonusStatus(startDate,bonusUnlockTime);
  ...
  return { player:players[0], startDate, released, completed, total:missions.length, current:current?clean(current):null, pendingPhoto:photoRows[0]?.status==="pending", rejectedPhotoReason:photoRows[0]?.status==="rejected"?String(photoRows[0]?.rejectionReason??"รูปยังไม่ผ่านการตรวจ"):null, latestUnlock:latest?{day:latest.day_number,tag:latest.tag,title:latest.title,clue:latest.clue,passedAt:latest.passedAt}:null, missions:missions.map(clean), nextMidnightMs:millisecondsToBangkokMidnight(),bonus:{...bonusTime,title:bonusTitle,unlockTime:bonusUnlockTime,sequenceRounds:Number(bonusSequenceRounds||8),codeAttempts:Number(bonusCodeAttempts||5),sequenceDone:Boolean(bonusRows[0]?.sequenceDone),codeDone:Boolean(bonusRows[0]?.codeDone),specialClue:Boolean(bonusRows[0]?.code_done)?bonusClue:null},finalAvailable:completed===missions.length&&!finalRows[0],finalCompleted:Boolean(finalRows[0]),finalMission:{title:finalTitle,task:finalTask,help:finalHelp,choices:typeof finalChoices==="string"?JSON.parse(finalChoices):finalChoices??[]} };
}
